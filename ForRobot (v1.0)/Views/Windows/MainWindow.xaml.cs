...
namespace ForRobot.Views.Windows
{
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            if (this.MainFrame.Content is Pages.PageMain1 && ((Pages.PageMain1)this.MainFrame.Content).ViewModel.Items.Count > 0)
            {
                foreach(var item in ((Pages.PageMain1)this.MainFrame.Content).ViewModel.Items)
                    if (((ViewModels.ToolBarViewModel)((Themes.ToolBarTrayForRobot)item).DataContext).Robot.IsConnection)
                    {
                        ...
                        {
                            foreach (var p in ((Pages.PageMain1)this.MainFrame.Content).ViewModel.Items)
                                if (!object.Equals(((ViewModels.ToolBarViewModel)p.DataContext).Robot, null) && !object.Equals(((ViewModels.ToolBarViewModel)p.DataContext).Robot.Connection, null)
                                    && ((ViewModels.ToolBarViewModel)p.DataContext).Robot.Connection.IsConnected) { ((ViewModels.ToolBarViewModel)p.DataContext).Dispose(); }
                            e.Cancel = false;
                        }
                        else
                            e.Cancel = true;
