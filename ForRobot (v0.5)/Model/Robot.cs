...
namespace ForRobot.Model
{
    public class Robot : BaseClass, IDisposable
    {
        ...
        private string _pathControllerField;
        ...
        {
            get => this._pathControllerField;
            set
            {
                this._pathControllerField = value;
                Json = JsonSerializer.Serialize<Robot>(this, options);
                if (!string.IsNullOrWhiteSpace(this._pathControllerField))
                    this.HostAndPort?.Invoke(this, null);
            }
        }
        ...
        public string Pro_State
        {
            ...
            set
            {
                ...
                RaisePropertyChanged("Pro_State");
            }
        }
                ...
                switch (this.Connection.Pro_State())
                {
                    case "#P_FREE":
                        if (!Task.Run<bool>(async () =>
                            await this.Connection.Copy(Path.Combine(this._pathControllerField, string.Join("", fileName, ".src")), Path.Combine(endPath, fileName))).Result)
                            new Exception($"Ошибка копирования файла {string.Join("", fileName, ".src")} из {this._pathControllerField} в {endPath}");
                        else
                            this.LogMessage($"Файл {string.Join("", fileName, ".src")} скопирован");

                        if (!Task.Run<bool>(async () =>
                            await this.Connection.Copy(Path.Combine(this._pathControllerField, string.Join("", fileName, ".dat")), Path.Combine(endPath, fileName))).Result)
                            new Exception($"Ошибка копирования файла {string.Join("", fileName, ".dat")} из {this._pathControllerField} в {endPath}");
                        else
        ...
        /// </summary>
        public void CopyToController(string yourePath, string fileName)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(this._pathControllerField))
