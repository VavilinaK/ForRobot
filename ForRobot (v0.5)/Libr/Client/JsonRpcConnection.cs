...
namespace ForRobot.Libr.Client
{
    public class JsonRpcConnection : IDisposable
    {
        ...
        /// </summary>
        public async Task<bool> CopyMem2File(string youreFilePath, string controllerPath)
        {
            this.LogMessage($"Копирование содержание файла {youreFilePath} в {controllerPath} . . .");

            if (!File.Exists(Path.Combine(youreFilePath)))
                this.LogErrorMessage($"Не существует файла {youreFilePath}");

            string content;
            using (StreamReader reader = new StreamReader(youreFilePath))
            {
