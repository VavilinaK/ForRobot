...
# форма детали d_rect, d_slope_left, d_slope_right, d_trapezoid_top, d_trapezoid_bottom
detail_params = {
    ...
    'h':200,                # высота ребра
    'd_w1':830,             # расстояние по ширине до осевой линии первого ребра
...
# a1 - начальный угол
# a2 - конечный угол
# speed - скорость сварки
def interpolate_weld_parm_by_speed(a1, a2, speed):
    min_spd = 24
    max_spd = 39
    min_amp = 2
    max_amp = 3.5
    i_spd = max(min_spd,min(speed,max_spd))
    proportion = (i_spd - min_spd)/(max_spd - min_spd)
    res_a = a1 + (a2 - a1) * (1-proportion**2)
    wave_len = i_spd/(max_spd/4.5)
    wave_amp = 2 + 1.5*(1-proportion)
    return [res_a, wave_len, wave_amp]
    
# генерация последовательности точек поиска для одного ребра
# start_point - начальная точка
# displace - смещение
# template - шаблон точек
# points - количество точек поиска
def gen_search_coord(start_point, displace, template, points = 3):
    d = displace
    search_gantry_radius = detail_params['search_gantry_radius']
    res = [ ]
    res.append([start_point + template['via'][0] + d, *template['via'][1:4], 100, 'via', search_gantry_radius])
    # в зависимости от кол-ва точек поиска генерируем выходной массив
    if points > 0:
        pts = [
            [start_point + template['s1'][0] + d, *template['s1'][1:4], 0.06, 's1', search_gantry_radius],
            [start_point + template['t1'][0] + d, *template['t1'][1:4], 0.02, 't1', 0]
        ]
        res += pts
    if points > 1:
        pts = [
            [start_point + template['s2'][0] + d, *template['s2'][1:4], 0.06, 's2', search_gantry_radius],
            [start_point + template['t2'][0] + d, *template['t2'][1:4], 0.02, 't2', 0]
        ]
        res += pts
    if points > 2:
        pts = [
            [start_point + template['s3v1'][0] + d, *template['s3v1'][1:4], 0.06, 's3v1', search_gantry_radius],
            [start_point + template['s3v2'][0] + d, *template['s3v2'][1:4], 0.02, 's3v2', 0],
            [start_point + template['t3'][0] + d, *template['t3'][1:4], 0.1, 't3', 0]
        ]
        res += pts
    res.append([start_point + template['departure'][0] + d, *template['departure'][1:4], 0.1, 'departure', search_gantry_radius])
...
# weld_len - длина шва
# template - шаблон точек
# departure_displace - смещение точки отхода
def gen_weld_coord(start_point, weld_len, template, departure_displace=ZERO_3D):
    ...
    res = [
        ...
        [start_point + template['weld'][0] + d, angle, *template['weld'][2:4], 0.01, 'arcoff', weld_gantry_radius],
        [start_point + template['near'][0] + d, angle, *template['near'][2:4],0.200 , 'away', weld_gantry_radius],
        [start_point + template['via'][0] + d + departure_displace, angle, *template['via'][2:4], 0.4, 'departure', weld_gantry_radius],
    ]
    #res = [
    ...
    #]
    #print(interpolate_weld_parm_by_speed(template['weld'][1], template['weld2'][1], detail_params['velocity']))
    # (x,y,z), (c,b,a), S, T, speed, type, gantry
    return res
...
# pos - текущая точка
# gantry_params - параметры позиционера
# prev_pos - предыдущая точка
def calc_gantry_for_pos(pos, gantry_params, prev_pos):
    ...
    # смещение фланца позиционера относительно центра шва для уменьшения вероятности попадания в сингулярность запястья робота
    if cba[2]>0:
        e3 += 200
    else:
        e3 -= 200   
...
# Генерация последовательности точек шва для одного ребра 
# edge_num - номер ребра
# side - сторона (left или right)
# seam_type - тип шва (start-to-middle или middle-to-end)   
def gen_seq_one_seam(edge_num, side, seam_type):
    d_type = detail_params['d_type']
    ...
    # положение осевой линии ребра по Y
    if (type(d_w2) == list): # разные расстояния между рёбрами
        r_y_pos = sum([d['d_dis'] for d in d_w2[:edge_num + 1]])
        d_l1 = d_w2[edge_num]['d_l1']
        d_l2 = d_w2[edge_num]['d_l2']
    else: # одинаковые расстояния между рёбрами
        r_y_pos = d_w1 + d_w2 * edge_num # осевая линия текущего ребра по Y

    # варианты формы детали
    ...
    if d_type == 'd_slope_left': # наклон влево
        corner2 = detail_params['d_b2'] # правый верхний угол
        corner3 = detail_params['d_b1'] # левый нижний угол
    elif d_type == 'd_slope_right': # наклон вправо
        corner1 = detail_params['d_b1'] # левый верхний угол
        corner4 = detail_params['d_b2'] # правый нижний угол
    ...
    if d_type != 'd_rect' and w != 0:
        bevel1 = corner1 + ((corner3 - corner1) / w) * r_y_pos
        bevel2 = corner2 + ((corner4 - corner2) / w) * r_y_pos
    #print(bevel1, bevel2)
    #длина шва = длина настила - расстояние до торцов - отступы - скосы
    r_len = l - d_l1 - d_l2 - l_r1 - l_r2 - (bevel1 + bevel2)
    #print(r_len)
    statements = []
    start_point = None
    if seam_type == 'middle-to-end':
        if side == 'left':
            ...
            # Z = тощина плиты
            # + координата 0 детали
            start_point = np.array([-d_l1 - l_r1 - r_len/2 + l_overlap - (corner1 + bevel1),
            ...
            w_left = gen_weld_coord(start_point, 
                                    r_len/2 + l_overlap - d_t2, 
                                    c_tmpl.weld_template_left,
                                    np.array([-l_r2 - 200, 0, 0]))
            gen_gantry(w_left)
        ...
        if side == 'left':
            ...
            start_point = np.array([-d_l1 - l_r1 - (corner1 + bevel1), 
                                    #d_w1 + d_w2 * edge_num - t_r/2, 
                                    r_y_pos - t_r/2,
                                    t_p]) + base
...
# Запись точки в src и dat файл
# src_f, dat_f ссылки на открытые для записи файлы
# p_name имя точки
# pos список параметров точки
# p_next_name имя следующей точки
# p_next список параметров следующей точки
...
# Сохранение файлов с KRL программой для одного ребра
# edge_num - номер ребра
# side - сторона (left или right)
# seam_type - тип шва (start-to-middle или middle-to-end)
# folder - путь к папке для сохранения файлов
...
def generate(out_dir, main_name, robot_num = 1):
    ...
    # проверка на соответствие количества рёбер для варианта с разными расстояниями между ними
    edge_count = detail_params['edge_count']
    d_w2 = detail_params['d_w2']
    if (type(d_w2) == list) and (edge_count != len(d_w2)):
        logging.info('Warning: edge_count != len(d_w2)')
        edge_count = len(d_w2)
        detail_params['edge_count'] = edge_count

    # формирование последовательности рёбер
    ...
    with open(main_f, 'w') as f:
        ...
        for side in ['right', 'left']:
            for edge_num in edge_seqence:
                seam_type = 'middle-to-end' # тип шва по умолчанию от центра до конца
                if robot_num == 1:
                    seam_type = 'start-to-middle' # тип шва для робота 1 от начала до центра    
...
if __name__ == '__main__':
    try:
        ...
        logging.info('Script version: ' + str(__version__))
