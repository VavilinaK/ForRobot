        ...
        protected override IList<Newtonsoft.Json.Serialization.JsonProperty> CreateProperties(Type type, MemberSerialization memberSerialization)
        {
            ...
            if(props.Count > 0 && props[0].DeclaringType.FullName == "ForRobot.Model.Detals.Plita")
            {
                if (this._scoseType == ScoseTypes.Rect)
                {
                    ...
                    props.Where(item => item.UnderlyingName == "BevelToStart").First().Ignored = true;
                    props.Where(item => item.UnderlyingName == "BevelToEnd").First().Ignored = true;
                }
                else
                {
                    props.Where(item => item.UnderlyingName == "Wight").First().Ignored = false;
                    props.Where(item => item.UnderlyingName == "BevelToStart").First().Ignored = false;
                    props.Where(item => item.UnderlyingName == "BevelToEnd").First().Ignored = false;
                }
